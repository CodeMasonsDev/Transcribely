...
import {
  ...
  workflowSteps,
  type AppScreen,
} from "../components/transcribely/shared";
import type {
  FailedClickUpTask,
              ...
              <div className="rounded-lg border border-slate-200">
                ...
                <div className="space-y-4 px-4 py-4">
                  <div className="space-y-2">
                    ...
                    <input
                      className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none transition focus:border-violet-600 focus:ring-4 focus:ring-violet-100"
                      id={`ticket-title-${ticket.id}`}
                      onChange={(event) =>
                        onUpdateTicket(ticket.id, { title: event.target.value })
                      }
                      type="text"
                      value={ticket.title}
                    />
                  ...
                  <div className="grid gap-4 md:grid-cols-[1fr_180px]">
                    ...
                    <div className="rounded-md border border-violet-100 bg-violet-50 px-3 py-2 text-xs leading-5 text-violet-800">
                      Changes update the ticket library immediately so review and
                      approval stay in sync.
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      className="min-h-32 w-full resize-y rounded-md border border-slate-300 bg-white px-3 py-2 text-sm leading-6 text-slate-900 outline-none transition focus:border-violet-600 focus:ring-4 focus:ring-violet-100"
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between gap-3">
                      <label
                        className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500"
                        htmlFor={`ticket-acceptance-${ticket.id}`}
                      >
                        Acceptance criteria
                      </label>
                      <span className="text-xs text-slate-500">
                        One field per criterion
                      </span>
                    </div>
                    <div className="space-y-3">
                      {ticket.acceptance_criteria.map((item, index) => (
                        <div className="flex items-start gap-2" key={`${ticket.id}-criterion-${index}`}>
                          <span className="mt-2 h-2.5 w-2.5 shrink-0 rounded-full bg-violet-500" />
                          <input
                            className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none transition focus:border-violet-600 focus:ring-4 focus:ring-violet-100"
                            onChange={(event) => {
                              const nextAcceptanceCriteria = [
                                ...ticket.acceptance_criteria.slice(0, index),
                                event.target.value,
                                ...ticket.acceptance_criteria.slice(index + 1),
                          ...
                          />
                          <button
                            className="mt-0.5 rounded-md border border-slate-200 bg-white px-2 py-1 text-xs font-semibold text-slate-600 hover:bg-slate-50"
                            onClick={() =>
                              onUpdateTicket(ticket.id, {
                      ...
                      <button
                        className="rounded-md border border-dashed border-violet-300 bg-violet-50 px-3 py-2 text-xs font-semibold text-violet-700 hover:bg-violet-100"
                        onClick={() =>
                          onUpdateTicket(ticket.id, {
                            acceptance_criteria: [
